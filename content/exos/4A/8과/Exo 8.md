# EXO 8 — 4A 과8
**Thème : Arrêter de fumer**

**I. Vocabulaire pour décrire des objets physiques**

*Contexte : un objet-gadget conçu comme message anti-tabac.*

1. Le [[cendrier|재떨이]] a la forme d'un [[poumon|폐]] : il est rond et dur.
2. Il est en plastique blanc avec un [[trou|구멍이 나다]] au milieu.
3. Quand on y [[fait tomber la cendre|담뱃재를 떨다]], il [[émet un son|소리를 내다]] : « [[keuf keuf|콜록콜록]] ».
4. La surface est douce mais le fond est dur.
5. C'est un [[bon exemple|대표적이다]] de [[gadget|아이디어 상품]] à message.
6. Ça rappelle que fumer est [[mauvais pour la santé|몸에 해롭다]] : le message est [[puissant|강력하다]].

**Corrections — I**
1. 재떨이가 폐 모양인데 둥글고 딱딱해요.
2. 하얀 플라스틱인데 가운데에 구멍이 나 있어요.
3. 담뱃재를 떨면 콜록콜록 소리를 내요.
4. 겉은 부드러운데 바닥은 딱딱해요.
5. 메시지를 담은 아이디어 상품의 대표적인 예예요.
6. 담배가 몸에 해롭다는 걸 알려 주는데 메시지가 강력해요.

**II. Comparer à une taille connue : noun만 해요**
1. Le [[cendrier|재떨이]] fait la taille de ce bol.
2. Le paquet fait la [[taille|크기]] de ma paume.
3. Le modèle réduit du [[poumon|폐]] fait la taille de ce ballon.
4. L'affiche fait la [[taille|크기]] de cette porte.
5. Le briquet fait la [[taille|크기]] de mon pouce.
6. Le patch fait la [[taille|크기]] de ce timbre.
7. La brochure fait la taille de ce [[carnet|수첩]].
8. La boîte de rangement fait la taille de ce [[tiroir|서랍]].
9. Le badge [[non-fumeur|금연]] fait la taille d'une pièce.
10. Le distributeur fait la [[taille|크기]] de ce réfrigérateur.

**Corrections — II**
1. 재떨이는 이 그릇만 해요.
2. 담뱃갑 크기가 손바닥만 해요.
3. 폐 모형은 이 공만 해요.
4. 포스터 크기가 이 문만 해요.
5. 라이터 크기가 제 엄지손가락만 해요.
6. 패치 크기가 이 우표만 해요.
7. 안내 책자는 이 수첩만 해요.
8. 보관함은 이 서랍만 해요.
9. 금연 배지는 동전만 해요.
10. 자판기 크기가 이 냉장고만 해요.

**III. Dépasser ou ne pas atteindre une valeur : -이 넘어요 / -이 안 돼요**
1. Le nombre de jours de [[non-tabac|금연]] dépasse cent.
2. Ça ne fait pas une semaine qu'il a [[arrêté de fumer|담배를 끊다]].
3. Depuis qu'il pense à ses [[poumons|폐]], le nombre de cigarettes par jour ne fait pas cinq.
4. Ça fait plus de deux ans qu'il [[a arrêté de fumer|담배를 끊다]].
5. Le taux de réussite pour [[arrêter de fumer|담배를 끊다]] ne fait pas dix pour cent.
6. Le prix d'un paquet, pourtant [[mauvais pour la santé|몸에 해롭다]], dépasse 4 500 wons.
7. La [[hauteur|높이]] du cendrier ne fait pas 8 cm.
8. Le [[poids|무게]] du cendrier ne fait pas 300 grammes.
9. Le nombre de campagnes cette [[année|올해]] dépasse vingt.
10. La [[largeur|가로]] de l'affiche dépasse un mètre.

**Corrections — III**
1. 금연한 날이 100일이 넘어요.
2. 그가 담배를 끊은 지 일주일이 안 돼요.
3. 폐 건강을 생각한 뒤로 하루에 피우는 담배가 다섯 개비가 안 돼요.
4. 그가 담배를 끊은 지 2년이 넘었어요.
5. 담배를 끊는 성공률이 10퍼센트가 안 돼요.
6. 몸에 해로운데도 한 갑 값이 4500원이 넘어요.
7. 재떨이 높이가 8센티미터가 안 돼요.
8. 재떨이 무게가 300그램이 안 돼요.
9. 올해 캠페인이 스무 개가 넘어요.
10. 포스터 가로가 1미터가 넘어요.
